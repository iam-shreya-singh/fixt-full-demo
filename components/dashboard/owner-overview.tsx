...
import { getOpenCapacity, getOwnerDashboard, type DashboardAppointment } from '@/app/actions/dashboard'
...
export function OwnerOverview() {
  ...
  useEffect(() => {
    ...
    getOpenCapacity().then((total) => { setCapacity(total); setCapacityState('ready') }).catch(() => setCapacityState('error'))
    getOwnerProfile().then((profile) => setSlug(profile?.slug ?? null)).catch(() => setSlug(null))
  }, [])
  ...
  const capacityValue = capacityState === 'loading' ? '...' : capacityState === 'ready' ? capacity.toString().padStart(2, '0') : '—'
  ...
  return <main className="min-h-screen bg-[#F7F4EC] px-5 py-6 md:px-10 md:py-8">
    <div className="mx-auto max-w-6xl">
      <header className="mb-10 flex flex-wrap items-end justify-between gap-6 border-b border-[#CBD1D7] pb-6">
        <div><p className={`${mono} mb-3 uppercase text-[#B87924]`}>Fixt / Owner workspace</p><h1 className={`font-serif text-5xl leading-none ${ink}`}>{getGreeting()}, {user?.firstName ?? 'there'}.</h1><p className="mt-4 text-sm leading-6 text-[#64748B]">Here&apos;s the shape of your day.</p></div>
        <div className="text-right"><p className={`${mono} uppercase text-[#64748B]`}>{formatDate(new Date().toISOString())}</p>{slug && <a href={`/book/${slug}`} target="_blank" rel="noreferrer" className="mt-3 inline-block border border-[#1E293B] bg-[#1E293B] px-4 py-2 text-sm text-[#F7F4EC] transition hover:bg-[#B87924]">View public booking page</a>}</div>
      </header>

      <section aria-label="Summary" className="grid border-y border-[#CBD1D7] md:grid-cols-3">
        {[['Today', today.length.toString().padStart(2, '0'), 'appointments'], ['Upcoming', upcoming.length.toString().padStart(2, '0'), 'next 7 days'], ['Open capacity', capacityValue, capacityState === 'loading' ? 'calculating slots...' : capacityState === 'error' ? 'unable to load slots' : 'slots remaining']].map(([label, value, detail]) => <div key={label} className="border-b border-[#CBD1D7] px-5 py-5 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0"><p className={`${mono} uppercase text-[#64748B]`}>{label}</p><p className={`mt-3 font-serif text-4xl ${ink}`}>{value}</p><p className="mt-1 text-sm text-[#64748B]">{detail}</p></div>)}
      </section>
